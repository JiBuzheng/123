 ...
 */
export function resolveAgentRouting(aiConfig: AiConfig, _token: string): AgentRouting {
  const isOllama = aiConfig.model === 'ollama';
  const isCustomApi = aiConfig.model === 'custom-api';
  const useCustomApi = isCustomApi && Boolean(aiConfig.apiKey?.trim() && aiConfig.endpoint?.trim());
  ...
  const selectedModel = selectedModelBase;
