...
export function useAgentRunner(options: UseAgentRunnerOptions): void {
  ...
  useEffect(() => {
    ...
    const routing = resolveAgentRouting(aiConfig, token);
    const canRunWithoutAccount = routing.isOllama
      || (routing.useCustomApi && aiConfig.customApiMode === 'direct');
    if ((!token && !canRunWithoutAccount) || !agentPrompt.trim()) {
      setPhase('error');
