...
const AI_API_KEY_STORE_KEY = 'ai-api-key';
...
function loadAiConfig(): AiConfig {
  ...
  try {
    ...
    if (raw) {
      ...
      if (merged.apiKey && window.api?.storeWrite) {
        void window.api.storeWrite(AI_API_KEY_STORE_KEY, merged.apiKey);
        localStorage.setItem(AI_CONFIG_KEY, JSON.stringify({ ...merged, apiKey: '' }));
      }
      return merged;
    }
  } catch { /* ignore */ }
  ...
  return {
    ...
    setAiConfig: (partial) => {
      ...
      if (Object.prototype.hasOwnProperty.call(partial, 'apiKey')) {
        void window.api?.storeWrite?.(AI_API_KEY_STORE_KEY, next.apiKey);
      }
      set({ aiConfig: next });
    },

    hydrateAiApiKey: async () => {
      try {
        const stored = await window.api?.storeRead?.(AI_API_KEY_STORE_KEY);
