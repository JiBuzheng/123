...
describe('registerStoreIpcHandlers', () => {
  ...
  describe('store:read', () => {
    ...
    it('migrates a legacy plaintext sensitive value to encrypted storage', () => {
      registerStoreIpcHandlers({ storeDir: 'C:/store' });
      existsSyncMock.mockReturnValue(true);
      readFileSyncMock.mockReturnValue(JSON.stringify('legacy-secret'));

      const result = handlers.get('store:read')!({}, 'ai-api-key');

      expect(result).toBe('legacy-secret');
      expect(writeFileSyncMock).toHaveBeenCalledWith(
        'C:/store/ai-api-key.json',
        expect.stringContaining('"__eislandEncrypted": 1'),
        'utf-8',
      );
      expect(writeFileSyncMock.mock.calls.at(-1)?.[1]).not.toContain('legacy-secret');
    });
  ...
  describe('store:write', () => {
    ...
    it('encrypts sensitive values before writing them to disk', () => {
      registerStoreIpcHandlers({ storeDir: 'C:/store' });
      const event = { sender: { id: 42 } };

      const result = handlers.get('store:write')!(event, 'mail-account-config', {
        authSecret: 'mail-password',
      });

      expect(result).toBe(true);
      const written = String(writeFileSyncMock.mock.calls.at(-1)?.[1]);
      expect(written).toContain('"__eislandEncrypted": 1');
      expect(written).not.toContain('mail-password');
    });
