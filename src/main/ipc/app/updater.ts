...
function applyUpdateSource(updater: AppUpdater, source: UpdateSourceKey, resolvedUrl?: string): void {
  ...
  if (source === 'tencent-cos' || source === 'aliyun-oss') {
    ...
    const trustedHost = parsedUrl.hostname === 'pyisland.com' || parsedUrl.hostname.endsWith('.pyisland.com');
    if (parsedUrl.protocol !== 'https:' || !trustedHost) {
      throw new Error('PRO update source URL is not trusted');
    }
    updater.setFeedURL({
      provider: 'generic',
      url: resolvedUrl,
...
export function registerUpdaterIpcHandlers(options: RegisterUpdaterIpcHandlersOptions): void {
  ipcMain.handle('updater:check', async (event, sourceRaw?: string, resolvedUrl?: string) => {
    if (!isTrustedIpcSender(event)) return { available: false, error: 'UNTRUSTED_IPC_SENDER' };
    try {
  ...
  ipcMain.handle('updater:download', async (event, sourceRaw?: string, resolvedUrl?: string) => {
  ...
  ipcMain.handle('guide:reset', (event) => {
    if (!isTrustedIpcSender(event)) return false;
